...
# durante o giro estoura o alcance máximo da perna e é "clampada" de volta,
# o que puxa o pé para cima no meio do passo — o robô sente isso como um
# coice/levantada da traseira. TRAS_TURN_SCALE reduz a amplitude em X das
# pernas traseiras somente durante o giro, mantendo a caminhada reta intacta.
TRAS_TURN_SCALE = 0.55
...
def frente_dir(self, stop_event, use_angular=True, delay_before_descent=0.0, sync_barrier=None, shared_state=None):
    ...
    while not stop_event.is_set():
        ...
        x_frente_eff = X_FRENTE * intensity
        x_atras_eff  = X_ATRAS * intensity
        ...
        swing_x, swing_z = _cubic_bezier(
            P0=[x_atras_eff * direction,  z_apoio_atual],   # decolagem (chão, atrás)
            P1=[x_atras_eff * direction,  Z_SWING],          # sobe rápido (vertical)
            P2=[x_frente_eff * direction, Z_SWING],          # mantém alto até a frente
            P3=[x_frente_eff * direction, z_apoio_atual],    # pouso (chão, frente)
            n=N_PONTOS,
        )
        ...
        for i, x in enumerate(np.linspace(x_frente_eff * direction, x_atras_eff * direction, N_PONTOS)):
            if stop_event.is_set():
                return
...
def tras_dir(self, stop_event, use_angular=True, delay_before_descent=0.0, sync_barrier=None, shared_state=None):
    ...
    while not stop_event.is_set():
        ...
        if shared_state is not None:
            ...
            if abs(yaw) > abs(speed):
                direction = -1 if yaw > 0 else 1  # perna direita recua ao girar para a direita
                # Atenua a amplitude traseira no giro: evita estourar MAX_RADIUS
                # (Z_APOIO_TRAS já está perto do limite) e o coice/levantada resultante.
                intensity = min(1.0, abs(yaw)) * TRAS_TURN_SCALE
            else:
                direction = shared_state.get("direction", 1)
                intensity = min(1.0, abs(speed))
        else:
        ...
        swing_x, swing_z = _cubic_bezier(
            P0=[x_atras_eff * direction,  z_apoio_atual],
            P1=[x_atras_eff * direction,  Z_SWING],
            P2=[x_frente_eff * direction, Z_SWING],
            P3=[x_frente_eff * direction, z_apoio_atual],
            n=N_PONTOS,
        )
        ang_swing = np.linspace(ANG_MAX, ANG_MIN, N_PONTOS)
        for i in range(N_PONTOS):
