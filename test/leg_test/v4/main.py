...
class RobotLeg:
    ...
    def test_stabilization(self):
        ...
        - Roll: ajusta os 4 servos angulares.
    ...
    def ps3_control_mode(self):
        ...
        try:
            ...
            with _SMBus(1) as _bus:
                ...
                # +X=BAIXO (ax=-1g), +Y=lateral (roll), +Z=cima (pitch)
                # roll=atan2(ay,-ax), pitch=atan2(-az,-ax) → 0° quando reto
                shared_state["imu_roll_offset"]  = _math.degrees(_math.atan2(_ay,  -_ax))
                shared_state["imu_pitch_offset"] = _math.degrees(_math.atan2(-_az, -_ax))
        ...
        def print_current_imu():
            try:
                ...
                with _SMBus(1) as _bus:
                    ...
                    _ay = _read_word(_bus, MPU6050_ADDR, ACCEL_XOUT_H + 2) / 16384.0
                    _az = _read_word(_bus, MPU6050_ADDR, ACCEL_XOUT_H + 4) / 16384.0
                    # +X=BAIXO, +Y=lateral, +Z=cima
                    roll_raw  = _math.degrees(_math.atan2(_ay,  -_ax))
                    pitch_raw = _math.degrees(_math.atan2(-_az, -_ax))
