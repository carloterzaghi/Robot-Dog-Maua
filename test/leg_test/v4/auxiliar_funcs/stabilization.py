...
Montagem do IMU (PCB Nova — parede lateral esquerda, corrigido com base nos eixos reais impressos):
  +X → TRÁS   (Oposto à frente do robô)
  +Y → BAIXO  (chão): ay = -1g quando nivelado (normal force para cima)
  +Z → DIREITA (para dentro do robô)
...
class _KalmanFilter:
    def __init__(self):
        self.Q_angle   = 0.001
        self.Q_bias    = 0.003
        self.R_measure = 0.10   # Aumentado (0.03→0.10): reduz ruído/variação na nova montagem vertical
        self.angle     = 0.0
        self.bias      = 0.0
...
def stabilize(robot_leg, stop_event):
    ...
    with SMBus(1) as bus:
        ...
        # Roll Positivo = Esquerda, Pitch Positivo = Nariz Cima
        ...
        while not stop_event.is_set():
            ...
            # Giroscópio (taxas de rotação)
            gx = _read_word(bus, MPU6050_ADDR, GYRO_XOUT_H)      / 131.0
            # gy não é usado para pitch/roll na nova orientação
            gz = _read_word(bus, MPU6050_ADDR, GYRO_XOUT_H + 4)  / 131.0

            # Mapeamento para eixos absolutos do robô
            accel_up      = -ay
            accel_right   = az
            accel_forward = -ax
            ...
            # Positivo = Roll Esquerda
            roll_acc  = math.degrees(math.atan2(accel_right, accel_up))
            roll      = kalman_roll.get_angle(roll_acc, gyro_roll_rate, dt)
            roll_norm = _clamp(roll / ROLL_MAX_DEG, -1.0, 1.0)
            
            # Para manter a lógica antiga de correção (onde Positivo = Roll Direita), invertemos o sinal:
            corr_roll = -roll_norm

            ang_frente_dir = _clamp(ANG_DIR_CENTER + corr_roll * ANG_DIR_RANGE,
                                    ANG_DIR_MIN, ANG_DIR_MAX)
            ang_frente_esq = _clamp(ANG_ESQ_CENTER + corr_roll * ANG_ESQ_RANGE,
            ...
            ang_tras_dir   = _clamp(ANG_ESQ_CENTER - corr_roll * ANG_ESQ_RANGE,
                                    ANG_ESQ_MIN, ANG_ESQ_MAX)
            ang_tras_esq   = _clamp(ANG_DIR_CENTER - corr_roll * ANG_DIR_RANGE,
                                    ANG_DIR_MIN, ANG_DIR_MAX)

            # ── Pitch via Kalman → fêmur e tíbia (IK) ────────────────────────
            # Positivo = Pitch Nariz Cima
            pitch_acc  = math.degrees(math.atan2(accel_forward, accel_up))
            pitch      = kalman_pitch.get_angle(pitch_acc, gyro_pitch_rate, dt)
            pitch_norm = _clamp(pitch / PITCH_MAX_DEG, -1.0, 1.0)

            # Pitch Positivo (Nariz Cima) -> frente retrai (Z fica mais próximo de 0), trás estende
...
def stabilize_angular(stop_event, robot_leg):
    ...
    try:
        with SMBus(1) as bus:
            ...
            # Seed: Mapeamento X=Trás, Y=Baixo, Z=Direita
            ax = _read_word(bus, MPU6050_ADDR, ACCEL_XOUT_H)     / 16384.0
            ay = _read_word(bus, MPU6050_ADDR, ACCEL_XOUT_H + 2) / 16384.0
            az = _read_word(bus, MPU6050_ADDR, ACCEL_XOUT_H + 4) / 16384.0
            accel_up    = -ay
            accel_right = az
            kalman.angle = math.degrees(math.atan2(accel_right, accel_up))
            ...
            while not stop_event.is_set():
                ...
                roll_acc  = math.degrees(math.atan2(accel_right, accel_up))
                roll      = kalman.get_angle(roll_acc, gyro_roll_rate, dt)
                roll_norm = _clamp(roll / ROLL_MAX_DEG, -1.0, 1.0)
    ...
    try:
        with SMBus(1) as bus:
            ...
            while not stop_event.is_set():
                ...
                roll_acc  = math.degrees(math.atan2(accel_right, accel_up)) - roll_offset
                roll      = kalman_roll.get_angle(roll_acc, gyro_roll_rate, dt)
                roll_norm = _clamp(roll / ROLL_MAX_DEG, -1.0, 1.0)
                ...
                pitch_acc  = math.degrees(math.atan2(accel_forward, accel_up)) - pitch_offset
                pitch      = kalman_pitch.get_angle(pitch_acc, gyro_pitch_rate, dt)
                pitch_norm = _clamp(pitch / PITCH_MAX_DEG, -1.0, 1.0)
